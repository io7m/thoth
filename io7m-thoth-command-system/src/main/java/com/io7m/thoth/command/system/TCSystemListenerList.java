...
/**
 * A command that displays the available listeners.
 */

@Component(immediate = true, service = ThothCommandType.class)
public final class TCSystemListenerList extends TCSystemCommand
{
  ...
  {
    if (text.size() == 1) {
      return this.resolver.listenerGroupList(text.get(0)).toList();
    }

    return List.of("usage: " + this.name() + " <group>");
  }
}
